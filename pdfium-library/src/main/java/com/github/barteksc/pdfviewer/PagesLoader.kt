...
class PagesLoader {
    ...
    private void loadVisible(String searchQuery) {
        int parts = 0;
        ...
        for (RenderRange range : rangeList) {
            ...
            parts += loadPage(range.page, range.leftTop.row, range.rightBottom.row, range.leftTop.col, range.rightBottom.col, CACHE_SIZE - parts, searchQuery);
            if (parts >= CACHE_SIZE) {
                break;
            }
        }
    ...
    private int loadPage(int page, int firstRow, int lastRow, int firstCol, int lastCol,
                         int nbOfPartsLoadable, String searchQuery) {
        int loaded = 0;
        for (int row = firstRow; row <= lastRow; row++) {
            for (int col = firstCol; col <= lastCol; col++) {
                if (loadCell(page, row, col, pageRelativePartWidth, pageRelativePartHeight, searchQuery)) {
                    loaded++;
                }
    ...
    private boolean loadCell(int page, int row, int col, float pageRelativePartWidth, float pageRelativePartHeight, String searchQuery) {
        ...
        if (renderWidth > 0 && renderHeight > 0) {
            if (!pdfView.cacheManager.upPartIfContained(page, pageRelativeBounds, cacheOrder, searchQuery)) {
                pdfView.renderingHandler.addRenderingTask(page, renderWidth, renderHeight,
                        ...
                        pdfView.isAnnotationRendering(), searchQuery);
            }
    ...
    private void loadThumbnail(int page, String searchQuery) {
        ...
        if (!pdfView.cacheManager.containsThumbnail(page, thumbnailRect, searchQuery)) {
            pdfView.renderingHandler.addRenderingTask(page,
                    ...
                    true, 0, pdfView.isBestQuality(), pdfView.isAnnotationRendering(), searchQuery);
        }
    }

    void loadPages(String searchQuery) {
        cacheOrder = 1;
