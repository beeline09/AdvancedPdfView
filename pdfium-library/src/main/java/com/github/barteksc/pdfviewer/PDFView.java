...
public class PDFView extends RelativeLayout {
    ...
    private void load(DocumentSource docSource, String password, int[] userPages) {
        ...
        if (!recycled) {
            throw new IllegalStateException(
                    "Don't call load on a PDF View without recycling it first.");
        }
    ...
    public void setNightMode(boolean nightMode) {
        ...
        if (nightMode) {
            ColorMatrix colorMatrixInverted = new ColorMatrix(
                    new float[] {-1, 0, 0, 0, 255, 0, -1, 0, 0, 255, 0, 0, -1, 0, 255, 0, 0, 0, 1,
                            0});
    ...
    private void drawWithListener(Canvas canvas, int page, OnDrawListener listener) {
        if (listener != null) {
            ...
            listener.onLayerDrawn(canvas, toCurrentScale(size.getWidth()),
                    toCurrentScale(size.getHeight()), page);
    ...
    private void drawPart(Canvas canvas, PagePart part) {
        ...
        RectF dstRect = new RectF((int) offsetX, (int) offsetY, (int) (offsetX + width),
                (int) (offsetY + height));
